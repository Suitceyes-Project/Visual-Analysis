...
This is the repository for the Suitceyes-Visual Analysis module. In visual analysis or VA, the incoming messages that contain live images from a remote camera are processed and analyzed. The analysis consists of 4 main areas, which are object detection and classification, face detection, classification and facemask detection and scene recognition. Various models were trained for these operations. For object detection, the OpenImages dataset has been used for the development of 2 models, the main one that is computationally heavy and a lighter one that can be placed on mobile devices. For scene recognition the Places dataset, has been utilized, for the development of 2 versions, the main one and a lighter, similarly to object detection. For face recognition the facenet library was utilized and for facemask detection the Real-World Masked Face and Simulated Masked Face datasets. The results from the aforementioned operations can be distributed through a messaging bus that handles the communication between the camera, the VA service and potentially any module that would require those results. These results consist of JSON files that describe the detected objects or human faces, their position and the class that they belong. In order to utilize the aforementioned platform capabilities of the module, a subscription to the said message bus (VA_KBS_channel) is required. The number of simultaneous subscriptions depends on the capabilities of the broker utilized.
