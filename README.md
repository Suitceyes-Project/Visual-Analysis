...
## Models
For object detection, the OpenImages dataset has been used for the development of 2 models, the main one that is computationally heavy and a lighter one that can be placed on mobile devices. For scene recognition the Places dataset, has been utilized, for the development of 2 versions, the main one and a lighter, similarly to object detection. For face recognition the facenet library was utilized and for facemask detection the Real-World Masked Face and Simulated Masked Face datasets. For access to the developed models contact Elias Kouslis (information below).
